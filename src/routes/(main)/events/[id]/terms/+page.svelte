<script lang="ts">
	...
	$: if (event) {
		setBreadcrumbs([
			...
			{
				text: m.terms(),
				href: `${PUBLIC_BASE_PATH}/events/${event.id}/terms`
			}
		]);
	}
...
<div class="flex items-center gap-4">
	...
	<h1 class="text-lg font-semibold md:text-2xl"> Editing Event Terms & Conditions </h1>
</div>
...
{#each terms as term, index}
	<Card.Root>
		<Card.Header>
			...
			<Card.Description>
				<Editor
					bind:value={terms[index].description}
...
<span> You can highlight text while editing to add formatting options. </span>
