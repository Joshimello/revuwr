<script lang="ts">
  ...
  const columns = table.createColumns([
    ...
    table.column({
      ...
      plugins: {
        sort: { getSortValue: (value) => value.year },
        filter: { getFilterValue: (value) => value.year },
        colFilter: { fn: ({ filterValue, value }) => filterValue.includes(value.year) }
      }
    }),
    table.column({
      id: 'applications',
...
<div class="flex items-center gap-2">
  ...
  />
  <Select.Root multiple onSelectedChange={selected => {
